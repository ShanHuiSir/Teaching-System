<template>
  <div class="page">
    ...
    <div class="card" style="position:relative;"><div class="card__body" style="padding:0;">
      <Snackbar v-if="snackbar.visible" :message="snackbar.message" :action-text="snackbar.actionText" :duration="snackbar.duration" status="ai" @close="snackbar.visible = false" @action="handleSnackbarAction" />
      <div v-if="loading" class="empty-state"><div class="spinner" /><div class="empty-state__text" style="margin-top:12px;">加载中...</div></div>
      <div v-else-if="!filtered.length" class="empty-state"><div class="empty-state__icon">&#129302;</div><div class="empty-state__text">暂无AI已审批作业</div></div>
      <table v-else class="table"><thead><tr><th>学生</th><th>作业标题</th><th>文件名</th><th>类型</th><th>AI评分</th><th>提交时间</th><th style="width:180px;">操作</th></tr></thead><tbody><tr v-for="s in filtered" :key="s.id"><td>{{ s.studentName }}</td><td>{{ s.title }}</td><td>{{ s.fileName }}</td><td>{{ s.workType }}</td><td>{{ evalMap[s.id]?.aiScore ?? '--' }}</td><td>{{ fmt(s.submittedAt) }}</td><td class="action-cell"><router-link class="btn btn--tonal btn--sm" :to="evalLink(s)">复核</router-link><button class="btn btn--text btn--sm" @click="doPreviewFile(s.id, s.fileName, s.contentType)">预览</button><button class="btn btn--text btn--sm" @click="doDownloadFile(s.id, s.fileName)">下载</button></td></tr></tbody></table>
    </div></div>
    <FloatingPreview
...
const filtered = computed(() => store.filteredByStatus(1));
...
watch(() => store.stats.aiReviewed, (now, prev) => {
  if (now > 0 && now > prev && prev >= 0) {
    snackbar.value = { visible: true, message: `${now}份作业待教师复核`, actionText: '查看', duration: 5000 };
  }
});
...
async function loadData() {
  try {
    ...
    statusMsg.value = `已加载 ${store.stats.aiReviewed} 条`;
